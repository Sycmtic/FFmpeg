...
static struct section sections[] = {
    ...
    [SECTION_ID_FRAME_VENC_PARAMS_BLOCK_DATA_LIST] ={ SECTION_ID_FRAME_VENC_PARAMS_BLOCK_DATA_LIST, "block_data_list", SECTION_FLAG_IS_ARRAY, { SECTION_ID_FRAME_VENC_PARAMS_BLOCK_DATA, -1 }, .element_name = "block_data", .unique_name = "frame_block_data_list"},
    [SECTION_ID_FRAME_VENC_PARAMS_BLOCK_DATA] =     { SECTION_ID_FRAME_VENC_PARAMS_BLOCK_DATA, "block_data", 0, { -1 }, .show_all_entries = 1 },
    ...
    if (frame->nb_side_data) {
        ...
        for (i = 0; i < frame->nb_side_data; i++) {
            ...
            if (sd->type == AV_FRAME_DATA_DISPLAYMATRIX && sd->size >= 9*4) {
                ...
                print_int("qp", par->qp);
                for (int plane = 0; plane < FF_ARRAY_ELEMS(par->delta_qp); plane++)
                    for (int acdc = 0; acdc < FF_ARRAY_ELEMS(par->delta_qp[plane]); acdc++) {
                        int delta_qp = par->delta_qp[plane][acdc];
                        if (delta_qp) {
                            char buf[16];
                            snprintf(buf, sizeof(buf), "delta_qp[%d][%d]", plane, acdc);
                            print_int(buf, delta_qp);
                        }
                    }
                if (par->nb_blocks) {
                    print_int("nb_blocks", par->nb_blocks);
                    writer_print_section_header(w, SECTION_ID_FRAME_VENC_PARAMS_BLOCK_DATA_LIST);
                    for (int i = 0; i < par->nb_blocks; i++) {
                        AVVideoBlockParams *b = av_video_enc_params_block(par, i);
                        writer_print_section_header(w, SECTION_ID_FRAME_VENC_PARAMS_BLOCK_DATA);
